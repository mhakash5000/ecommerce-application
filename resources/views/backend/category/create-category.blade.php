        ...
        <div class="col-md-6 offset-3 pt-3">
            <form action="{{(@$editData)?route('categories.update',$editData->id):route('categories.store')}} " method="POST" enctype="multipart/form-data">
                ...
                {{-- <div class="form-group">
                  ...
                  <textarea name="details" id="" cols="73" rows="5"  type="text" placeholder="Write Category Details" required>{{@$editData->details}}</textarea>
                  <font style="color:red">{{($errors->has('details'))?($errors->first('details')):''}} </font>
              </div>
              <div class="form-group">
                <label for="my-input">Image</label>
                <img src="{{(!empty($editData->image))?url('public/images/category_image/'.$editData->image):url('public/images/not_found_img.png')}}" id="image" style="width:530px;height:200px">
